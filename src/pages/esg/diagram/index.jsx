...
const CompleteDiagram = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 md:p-8">
      <main className="bg-bgLight dark:bg-bgDark rounded-[2rem] shadow-2xl p-6 md:p-12 w-full max-w-4xl border border-slate-100 relative overflow-hidden">
        {/* Header Section */}
        <header className="relative z-10 mb-8 text-center">
          <h1 className="text-3xl font-bold tracking-tight md:text-4xl text-slate-800 dark:text-brandAccent">
            ESG Stakeholder Ecosystem
          </h1>
          <p className="mt-2 text-sm font-medium text-slate-500 dark:text-white/70 md:text-base">
        ...
        <section className="grid grid-cols-1 gap-4 pt-8 mt-12 border-t border-slate-100 md:grid-cols-2 lg:grid-cols-3">
          {STAKEHOLDERS.map((s) => (
            <div key={`legend-${s.id}`} className="flex items-start gap-3 p-2 transition-colors rounded-lg cursor-default hover:bg-slate-50 dark:hover:bg-slate-800 group">
              <div
                className="w-3 h-3 rounded-full mt-1.5 flex-shrink-0 transition-transform group-hover:scale-125"
                style={{ backgroundColor: s.color }}
              />
              <div className="flex flex-col">
                <span className="text-xs font-bold text-slate-700 dark:text-white">{s.title}</span>
                <span className="text-[10px] text-slate-500 dark:text-white leading-tight">{s.description.split('.')[0]}.</span>
              </div>
            </div>
          ))}
        </section>
