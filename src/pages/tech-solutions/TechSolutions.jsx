...
const FeatureScreenshotCard = ({ label, title, description, image, delay = 0 }) => {
  ...
  return (
    <div
      ...
      style={{ transitionDelay: `${delay}ms` }}
    >
      {/* Card with browser chrome - dark theme support */}
      <div className="flex-1 overflow-hidden transition-shadow duration-300 shadow-lg bg-brandLight dark:bg-surfaceDark rounded-3xl hover:shadow-xl dark:shadow-brandAccent/10">
        {/* Browser header bar */}
        <div className="flex items-center gap-2 px-4 py-3 bg-brandPrimary dark:bg-brandNavy">
          <div className="flex gap-1.5">
            <div className="w-3 h-3 rounded-full bg-white/30" />
            <div className="w-3 h-3 rounded-full bg-white/30" />
            <div className="w-3 h-3 rounded-full bg-white/30" />
        ...
        {/* Screenshot content area */}
        <div className="p-4 bg-white dark:bg-bgDark">
          <p className="mb-1 text-xs font-semibold tracking-wide uppercase text-brandAccent">{label}</p>
          <h4 className="mb-3 text-base font-bold font-heading text-brandDark dark:text-white">{title}</h4>
          
          {/* Screenshot image */}
          <div className="overflow-hidden border rounded-xl border-borderLight dark:border-borderDark">
            <img 
              src={image} 
              alt={title} 
              className="object-cover object-top w-full h-52"
      ...
      {/* Title and CTA below card */}
      <div className="mt-6">
        <h3 className="mb-3 text-xl font-bold font-heading text-brandDark dark:text-white">{description}</h3>
        <button className="inline-flex items-center gap-2 px-6 py-3 text-sm font-semibold transition-colors duration-300 rounded-full bg-brandAccent text-brandDark hover:bg-brandGold hover:shadow-lg hover:shadow-brandAccent/30">
          Try it now
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
...
const BentoCard = ({ variant = 'light', title, description, image, buttonText,...other }) => {
  const bgClass = variant === 'light' 
    ? 'bg-brandLight dark:bg-surfaceDark' 
    : 'bg-gradient-to-br from-brandDark to-brandNavy dark:from-brandNavy dark:to-brandDark';
  const textClass = variant === 'light' ? 'text-brandDark dark:text-white' : 'text-white';
  
  return (
    <div {...other} className={`${bgClass} rounded-3xl p-8 lg:p-10 h-full flex flex-col border border-transparent dark:border-borderDark`}>
      <h3 className={`text-2xl lg:text-3xl font-heading font-bold ${textClass} mb-4`}>
        {title}
      </h3>
      <p className={`${variant === 'light' ? 'text-brandDark/70 dark:text-white/70' : 'text-white/70'} mb-6 text-base leading-relaxed`}>
        {description}
      </p>
      {buttonText && (
        <button className={`inline-flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm
          ${variant === 'light' 
            ? 'bg-brandPrimary text-white hover:bg-brandNavy dark:bg-brandAccent dark:text-brandDark dark:hover:bg-brandGold' 
            : 'bg-brandAccent text-brandDark hover:bg-brandGold'}
          transition-colors duration-300 w-fit mb-6`}>
          {buttonText}
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {image && (
        <div className="flex items-end flex-1">
          <div className="w-full overflow-hidden border shadow-2xl rounded-xl border-borderLight dark:border-borderDark dark:shadow-brandAccent/5">
            <img src={image} alt={title} className="w-full h-auto" />
          </div>
        </div>
      )}
...
const SolutionIconCard = ({ icon, title, description, delay = 0 }) => {
  ...
  return (
    <div
      ...
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className="flex items-center justify-center mx-auto mb-4 bg-white border border-transparent shadow-lg dark:bg-surfaceDark dark:shadow-brandAccent/10 w-14 h-14 rounded-xl dark:border-borderDark">
        {icon}
      </div>
      <h4 className="mb-2 text-lg font-bold text-white font-heading">{title}</h4>
      <p className="text-sm leading-relaxed text-white/60">{description}</p>
...
const ScreenshotShowcase = ({ image, title, description, features, reverse = false }) => {
  ...
  return (
    ...
    >
      <div className={`${reverse ? 'lg:order-2' : ''}`}>
        <h3 className="mb-4 text-3xl font-bold font-heading text-brandDark dark:text-white">{title}</h3>
        <p className="mb-6 text-base leading-relaxed text-brandDark/70 dark:text-white/70">{description}</p>
        <ul className="space-y-3">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-3">
              <div className="w-6 h-6 rounded-full bg-brandAccent/20 dark:bg-brandAccent/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                <svg className="w-3.5 h-3.5 text-brandAccent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                </svg>
              </div>
              <span className="text-sm text-brandDark/80 dark:text-white/80">{feature}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className={`${reverse ? 'lg:order-1' : ''}`}>
        <div className="relative">
          <div className="absolute -inset-4 bg-gradient-to-r from-brandPrimary/10 to-brandAccent/10 dark:from-brandAccent/5 dark:to-brandPrimary/5 rounded-3xl blur-2xl" />
          <div className="relative overflow-hidden border shadow-2xl rounded-2xl border-borderLight dark:border-borderDark dark:shadow-brandAccent/10">
            <img src={image} alt={title} className="w-full" />
          </div>
        </div>
      </div>
...
// ============================================================
const PlanFeatureCard = ({ icon, title }) => (
  <div className="p-6 transition-all duration-300 bg-white border dark:bg-surfaceDark rounded-2xl border-borderLight dark:border-borderDark hover:shadow-lg hover:shadow-brandPrimary/5 dark:hover:shadow-brandAccent/10">
    <div className="w-10 h-10 mb-4">{icon}</div>
    <p className="text-sm font-medium leading-relaxed text-brandDark dark:text-white">{title}</p>
  </div>
);
...
// ============================================================
const TestimonialCard = ({ quote, name, rating = 5 }) => (
  <div className="bg-brandLight dark:bg-surfaceDark rounded-2xl p-6 min-w-[300px] max-w-[350px] border border-borderLight dark:border-borderDark">
    <p className="mb-6 text-sm leading-relaxed text-brandDark/80 dark:text-white/80">"{quote}"</p>
    <div className="flex items-center gap-3">
      <div className="flex items-center justify-center w-10 h-10 rounded-full bg-brandPrimary/20 dark:bg-brandAccent/20">
        <span className="text-sm font-bold text-brandPrimary dark:text-brandAccent">{name.charAt(0)}</span>
      </div>
      <div>
        <div className="flex gap-0.5 mb-1">
          {[...Array(rating)].map((_, i) => (
        ...
        <p className="text-sm font-semibold text-brandDark dark:text-white">{name}</p>
      </div>
    </div>
  </div>
);
...
// ============================================================
const FAQItem = ({ question, answer, isOpen, onClick }) => (
  <div className="overflow-hidden border bg-brandLight dark:bg-surfaceDark rounded-2xl border-borderLight dark:border-borderDark">
    <button
      onClick={onClick}
      className="flex items-center justify-between w-full px-6 py-5 text-left transition-colors hover:bg-brandLight/80 dark:hover:bg-surfaceDark/80"
    >
      <span className="pr-4 font-medium text-brandDark dark:text-white">{question}</span>
      <div className={`w-8 h-8 rounded-full bg-brandPrimary/10 dark:bg-brandAccent/20 flex items-center justify-center flex-shrink-0 
        transition-transform duration-300 ${isOpen ? 'rotate-45' : ''}`}>
        <svg className="w-4 h-4 text-brandPrimary dark:text-brandAccent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
        </svg>
      </div>
    </button>
    <div className={`overflow-hidden transition-all duration-300 ${isOpen ? 'max-h-48' : 'max-h-0'}`}>
      <p className="px-6 pb-5 text-sm leading-relaxed text-brandDark/70 dark:text-white/70">{answer}</p>
    </div>
  </div>
);
...
export default function IntegritatSolutionPage() {
  ...
  return (
    <div className="min-h-screen font-sans bg-white dark:bg-bgDark">
      
      {/* ==================== HERO SECTION ==================== */}
      <section className="relative flex items-center min-h-screen overflow-hidden bg-bgLight dark:bg-brandDark">
        <div className="absolute inset-0">
          <div className="absolute rounded-full top-20 right-20 w-96 h-96 bg-brandAccent/10 dark:bg-brandAccent/5 blur-3xl" />
          <div className="absolute rounded-full bottom-20 left-20 w-96 h-96 bg-brandPrimary/10 dark:bg-brandPrimary/5 blur-3xl" />
        </div>

        <div className="container relative z-10 px-6 py-20 mx-auto lg:px-20">
          <div className="grid items-center gap-16 lg:grid-cols-2">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 border rounded-full bg-brandPrimary/10 dark:bg-brandAccent/10 border-brandPrimary/20 dark:border-brandAccent/20">
                <span className="w-2 h-2 rounded-full bg-brandAccent animate-pulse" />
                <span className="text-sm font-medium text-brandPrimary dark:text-brandAccent">Audit Management Platform</span>
              </div>
              
              <h1 className="mb-6 text-4xl font-bold leading-tight lg:text-5xl xl:text-6xl font-heading text-brandDark dark:text-white">
                Streamline Your{' '}
                <span className="text-brandPrimary dark:text-brandAccent">Audit Workflow</span>
              </h1>
              
              <p className="max-w-lg mb-8 text-lg leading-relaxed text-brandDark/70 dark:text-white/70">
                CPA firms can automate document collection, tracking, and review — 
                everything in between from within Integritat.
              </p>
              
              <div className="flex flex-wrap gap-4 mb-12">
                <Link to ="/contact" className="px-8 py-4 font-semibold transition-all duration-300 rounded-full shadow-lg text-brandDark bg-brandAccent hover:bg-brandGold shadow-brandAccent/30">
                  Start your free trial
                </Link>
              </div>
            </div>
            ...
            {/* Hero Screenshot */}
            <div className="relative">
              <div className="absolute -inset-4 bg-gradient-to-r from-brandPrimary/20 to-brandAccent/20 dark:from-brandAccent/10 dark:to-brandPrimary/10 rounded-3xl blur-2xl" />
              <div className="relative overflow-hidden border shadow-2xl rounded-2xl border-borderLight dark:border-borderDark dark:shadow-brandAccent/10">
                <img src={SCREENSHOTS.hero} alt="Integritat Login" className="w-full" />
              </div>
              
              {/* Floating dashboard preview */}
              <div className="absolute w-48 overflow-hidden transition-transform duration-300 transform border shadow-xl -bottom-6 -left-6 rounded-xl border-borderLight dark:border-borderDark hover:scale-105 animate-float dark:shadow-brandAccent/10">
                <img src={SCREENSHOTS.mainDash} alt="Dashboard" className="w-full" />
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-surfaceLight dark:bg-surfaceDark">
        <div className="container px-6 mx-auto lg:px-12">
          <div className="max-w-3xl mx-auto mb-16 text-center">
            <h2 className="mb-6 text-4xl font-bold lg:text-5xl font-heading text-brandDark dark:text-white">
              Complete Audit Workflow in{' '}
              <span className="text-brandPrimary dark:text-brandAccent">8 Simple Steps</span>
            </h2>
            <p className="text-base text-brandDark/70 dark:text-white/70">
              From client onboarding to governance dashboard — manage your entire audit lifecycle seamlessly
            </p>
          </div>
      ...
      <section className="relative py-24 overflow-hidden bg-gradient-to-br from-brandDark to-brandNavy">
        ...
        <div className="container relative z-10 px-6 mx-auto lg:px-12">
          <div className="mb-16 text-center">
            ...
            <p className="max-w-2xl mx-auto text-base text-white/60">
              Offers a unified platform that fosters collaboration while providing end-to-end audit management.
            </p>
          </div>
          ...
          <div className="grid gap-12 mb-16 md:grid-cols-2 lg:grid-cols-3">
            <SolutionIconCard
              icon={<svg className="w-7 h-7 text-brandPrimary dark:text-brandAccent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
              title="Fast and Reliable"
              description="Whether you have a team of 2 or 200, our platform keeps everyone on the same page."
              delay={0}
            />
            ...
            />
            <SolutionIconCard
              icon={<svg className="w-7 h-7 text-brandPrimary dark:text-brandAccent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
              title="Enterprise Security"
              description="Role-based access control for complete compliance."
              delay={200}
            />
      ...
      {/* ==================== FAQ ==================== */}
      <section className="py-24 bg-brandLight dark:bg-surfaceDark">
        <div className="container px-6 mx-auto lg:px-12">
          <h2 className="mb-12 text-4xl font-bold text-center font-heading text-brandDark dark:text-white">
            Frequently asked questions!
          </h2>
      ...
      {/* ==================== CTA SECTION ==================== */}
      <section className="relative py-32 overflow-hidden bg-surfaceLight dark:bg-brandDark/30">
        <div className="absolute inset-0">
          <div className="absolute top-20 left-[15%] w-14 h-14 rounded-full bg-brandPrimary/10 dark:bg-brandAccent/10 flex items-center justify-center animate-float">
            <svg className="w-6 h-6 text-brandPrimary dark:text-brandAccent" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>
          </div>
          <div className="absolute top-32 right-[20%] w-16 h-16 rounded-full bg-brandAccent/20 flex items-center justify-center animate-float" style={{ animationDelay: '1s' }}>
            <svg className="w-7 h-7 text-brandAccent" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>
          </div>
          <div className="absolute bottom-32 left-[25%] w-12 h-12 rounded-full bg-brandNavy/10 dark:bg-brandPrimary/10 flex items-center justify-center animate-float" style={{ animationDelay: '2s' }}>
            <svg className="w-5 h-5 text-brandNavy dark:text-brandPrimary" fill="currentColor" viewBox="0 0 24 24"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>
          </div>
          <div className="absolute bottom-20 right-[15%] w-14 h-14 rounded-full bg-brandGold/20 flex items-center justify-center animate-float" style={{ animationDelay: '0.5s' }}>
            <svg className="w-6 h-6 text-brandGold dark:text-brandAccent" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
          </div>
        </div>

        <div className="container relative z-10 px-6 mx-auto lg:px-12">
          <div className="max-w-3xl mx-auto text-center">
            <span className="inline-block px-4 py-2 mb-6 text-sm font-medium border rounded-full border-borderLight dark:border-borderDark text-brandDark dark:text-white">
              Get started today!
            </span>
            <h2 className="mb-6 text-4xl font-bold lg:text-5xl font-heading text-brandDark dark:text-white">
              Seamless integration with your{' '}
              <span className="text-brandPrimary dark:text-brandAccent">existing workflow</span>
            </h2>
            <p className="mb-8 text-base text-brandDark/70 dark:text-white/70">
              See how we help your team solve today's biggest audit challenges.
            </p>
            <button onClick={()=>navigator("/contact")} className="px-10 py-5 text-lg font-semibold transition-all duration-300 rounded-full shadow-xl text-brandDark bg-brandAccent hover:bg-brandGold shadow-brandAccent/30">
              Start your free trial
            </button>
            <div className="flex items-center justify-center gap-2 mt-6 text-sm text-brandDark/60 dark:text-white/60">
              <div className="flex gap-0.5">
              </div>
            </div>
          </div>
