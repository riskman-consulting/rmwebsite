...
const Testimonials = () => {
  ...
  // Shared navigation dots markup
  const NavDots = ({ extraClass = "" }) => (
      ...
      {testimonials.map((_, idx) => (
        <button
          ...
          className={`h-3 rounded-full transition-all duration-300 ${
            idx === activeIndex
              ? "bg-brandAccent w-8"
              : "bg-brandNavy/20 dark:bg-white/20 hover:bg-brandNavy/40 w-3"
          }`}
      ...
      <div className="w-full px-6 mx-auto lg:px-20">
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
          ...
          <div className="max-w-xl mx-auto text-center lg:text-left lg:mx-0">
            ...
            <h2 className="mb-8 text-3xl font-black leading-tight md:text-5xl lg:text-4xl text-brandDark dark:text-white">
              What Our{" "}
            ...
            {/* Desktop-only navigation dots — left-aligned inside left column */}
            <div className="hidden lg:block">
              <NavDots />
            </div>
          </div>

          {/* RIGHT SIDE - Stacked Carousel + Mobile Nav */}
          <div className="flex flex-col items-center gap-0">
            {/* Card stack */}
            <div className="relative w-full h-[380px] md:h-[440px] lg:h-[500px] flex items-center justify-center perspective-1000">
                ...
                {testimonials.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Text */}
                      <p
                        className={`mb-6 md:mb-8 text-base md:text-lg font-medium leading-relaxed ${item.textColor} opacity-90`}
                      >
                        "{item.text}"
                      </p>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4
                            className={`text-xs md:text-sm font-bold uppercase tracking-wider ${item.textColor}`}
                          >
                            {item.name}
                          </h4>
                          <p
                            className={`text-[10px] md:text-xs font-medium opacity-70 ${item.textColor}`}
                          >
                            {item.role.split(",")[0]}
                          </p>
                        </div>
            ...
            {/* Mobile-only navigation dots — centered below cards */}
            <div className="flex justify-center w-full pt-6 lg:hidden">
              <NavDots />
            </div>
          </div>
