...
const OperatingModel = () => {
    ...
    return (
        <section id="model" className="relative z-10 py-24 overflow-hidden bg-bgDark">
            ...
            <div className="container relative z-10">
                ...
                <div className="relative grid grid-cols-1 gap-4 md:grid-cols-5">
                    {phases.map((phase, index) => (
                        ...
                        >
                            <div className="flex flex-col items-center h-full p-8 text-center transition-all duration-300 border bg-surfaceDark/40 backdrop-blur-sm border-brandGold/10 rounded-2xl group-hover:bg-surfaceDark group-hover:border-brandGold/30 group-hover:-translate-y-2 group-hover:shadow-2xl">
                                <div className="font-mono text-[0.6rem] tracking-[3px] text-brandAccent uppercase mb-6 font-bold opacity-60 group-hover:opacity-100">
                                    Phase {phase.number}
                                </div>
                                <div className="w-14 h-14 mb-6 border border-brandGold/30 rounded-full flex items-center justify-center bg-brandDark shadow-[0_0_20px_rgba(255,184,0,0.1)] group-hover:border-brandGold group-hover:shadow-[0_0_30px_rgba(255,184,0,0.2)] transition-all">
                                    {phase.icon}
                                </div>
                                <div className="font-heading text-[0.95rem] font-bold text-brandLight mb-3 leading-tight">
                                    {phase.title}
                                </div>
                                <div className="font-sans text-xs font-light leading-relaxed text-brandLight/50">
                                    {phase.desc}
                                </div>
                            </div>

                            {index < phases.length - 1 && (
                                <div className="absolute z-20 hidden transition-colors -translate-y-1/2 md:flex -right-3 top-1/2 text-brandGold/30 group-hover:text-brandGold">
                                    <ArrowRight size={24} />
                                </div>
                            )}
                        </motion.div>
                ...
                >
                    <div className="flex items-center w-full max-w-2xl gap-4 px-4">
                        ...
                        <div className="text-[0.7rem] font-mono uppercase tracking-[3px] text-brandAccent flex items-center gap-3">
