                              ...
                              >
                                ...
                                <ul className="space-y-1.5">
                                  {section.items.map((sub, j) => {
                                    const hasNestedSubmenu = sub.submenus && sub.submenus.length > 0;
                                    return (
                                      <li key={j} className="relative group">
                                        <a
                                          href={sub.path}
                                          className="text-sm transition-colors text-[#001F3F] dark:text-[#F5F5F5] hover:text-[#004080] dark:hover:text-[#FFC000] block py-1 flex items-center justify-between"
                                        >
                                          <span>{sub.label || sub.lable}</span>
                                          {hasNestedSubmenu && <ChevronDown size={12} className="opacity-50 group-hover:opacity-100" />}
                                        </a>
                                       
                                        {/* Nested Submenu */}
                                        {hasNestedSubmenu && (
                                          <div className="absolute left-full top-0 ml-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 bg-white dark:bg-[#001122] rounded-lg shadow-lg border border-[rgba(0,31,63,0.1)] dark:border-[rgba(255,192,0,0.15)] p-3 z-[110]">
                                            {sub.submenus.map((nested, k) => (
                                              <a
                                                key={k}
                                                href={`${sub.path}/${nested.id || nested.path}`}
                                                className="block text-xs py-2 px-2 text-[#001F3F] dark:text-[#F5F5F5] hover:text-[#004080] dark:hover:text-[#FFC000] hover:bg-[rgba(0,31,63,0.05)] dark:hover:bg-[rgba(255,192,0,0.1)] rounded transition-colors"
                                              >
                                                {nested.title || nested.label || nested.lable}
                                              </a>
                                            ))}
                                          </div>
                                        )}
